...
vignette: >
    %\VignetteIndexEntry{LIME tutorial}
...
```
**Update**: Some cells of this vignette have been prevented from running as there is a problem with LIME and its
`Matrix` package dependency. A fix is coming soon. More details on [Readme](https://d2gex.github.io/lbmstoolbox/).
```{r, setup}
...
```{r lime_simulation_catch, eval = FALSE}
result_lime$simulation$model_info$catch %>%
  head(3)
```

```{r lime_simulation_population, eval = FALSE}
...
```{r lbspr_estimate_plots, eval = FALSE}
...
```{r lime_estimate_catch_plots, eval = FALSE}
...
```{r lime_estimate_population_plots, eval = FALSE}
